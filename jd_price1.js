  ...
  (async () => {
    ...
    try {
      ...
      $.logErr(err.message || $.toStr(err));
       const clickAction = `const url='https://item.jd.com/${productId}.html'; if(navigator.clipboard){navigator.clipboard.writeText(url).finally(()=>{window.location.href='manmanbuy://'})}else{window.location.href='manmanbuy://'}`;
       const errorHtml = `<div style="padding: 20px; text-align: center; color: red; font-size: 16px;"><strong>比价失败：</strong><span onclick="${clickAction}" style="cursor: pointer; text-decoration: underline;">${err.message || '未知错误'}</span></div>`;
